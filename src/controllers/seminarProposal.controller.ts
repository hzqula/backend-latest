...
export const assessProposalSeminar: RequestHandler = async (
  ...
  try {
    ...
    try {
      const seminarDetails = await seminarProposalService.getSeminarDetails(parseInt(id));
      ...
      if (email) {
        ...
        console.error('Email dosen tidak tersedia untuk logging riwayat penilaian');
      }
    } catch (emailError) {
