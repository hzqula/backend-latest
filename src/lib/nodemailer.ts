...
export const sendAssessmentEmail = async ({
  ...
  const formattedSeminarType =
    seminarType.charAt(0).toUpperCase() + seminarType.slice(1).toLowerCase();

  // Tentukan subject dan roleText berdasarkan lecturerRole
  const subject =
    lecturerRole === "ADVISOR"
    ...
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
        ...
        <ul style="font-size: 14px; color: #000; list-style: none; padding: 0;">
            ...
            ${
              lecturerRole === "ASSESSOR"
                ? `<li><strong>Nilai Penulisan:</strong> <strong>${writingScore}</strong></li>`
                : ""
            }
            ...
            ${
              lecturerRole === "ADVISOR"
                ? `<li><strong>Nilai Karakteristik:</strong> <strong>${
                    characteristicScore !== null
                      ? characteristicScore
                      : "Tidak Ada"
                  }</strong></li>`
                : ""
